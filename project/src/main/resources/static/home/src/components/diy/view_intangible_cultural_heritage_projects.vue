	...
	export default {
		...
		methods: {
			...
			async get_group_user_inheritance_personnel() {
							this.form["inheritance_personnel"] = this.$store.state.user.user_id;
          ...
          this.form["announcement_date"] = this.$toTime(parseInt(this.form["announcement_date"]),"yyyy-MM-dd")
			...
			get_obj_after(json, func){
																																				...
																																				if(json.result.obj["announcement_date"]=="0000-00-00"){
				  json.result.obj["announcement_date"] = null;
				}
				if(parseInt(json.result.obj["announcement_date"]) > 9999){
					json.result.obj["announcement_date"] = this.$toTime(parseInt(json.result.obj["announcement_date"]),"yyyy-MM-dd")
				}
