<template>
	<div class="diy_details_box diy_div_intangible_cultural_heritage_projects">
		<div class="warp diy_warp">
			<div class="container">
					<div class="details_title">{{ obj.entry_name || "详情" }}</div>
				<div class="row row_det">
						<div v-if="this.$store.state.user.user_id" class="btns_add col-12 col-sm-6">
							<div v-if="$check_action('/intangible_cultural_heritage_projects/details','set')">
								<!-- 点赞按钮 -->
								<div
										v-if="state_praise"
						...
						<!-- 图片 -->
							...
							<div class="row_cont" :class="{flex_row2_item: !imgList.length}" v-for="(item,index) in itemList" :key="item+index" v-show="$check_field('get',item.name)">
								...
								<div class="diy_field diy_text" :class="{flex_row2_item_text:imgList.length}" v-else-if="item.type == '传承人'">
									<span>{{ inheritorDisplay }}</span>
								</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async loadInheritorNames() {
				if (!this.obj || !this.obj.entry_name) {
					this.inheritorNames = [];
					return;
				}
				const names = [];
				if (this.obj.personnel_name) {
					names.push(this.obj.personnel_name);
				}
				const json = await this.$get("~/api/inheritance_personnel/get_list?like=0", {
					page: 1,
					size: 200,
					entry_name: this.obj.entry_name
				});
				if (json.result && json.result.list) {
					json.result.list.forEach((item) => {
						if (item.personnel_name && names.indexOf(item.personnel_name) === -1) {
							names.push(item.personnel_name);
						}
					});
				}
				if (this.obj.inheritance_personnel) {
					const sub = await this.$get("~/api/inheriting_users/get_obj?", { user_id: this.obj.inheritance_personnel });
					if (sub.result && sub.result.obj && sub.result.obj.personnel_name) {
						const n = sub.result.obj.personnel_name;
						if (names.indexOf(n) === -1) names.push(n);
					}
				}
				this.inheritorNames = names;
			},
				/**
			 * 改变点赞数
			 */
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "intangible_cultural_heritage_projects_id",
					source_id: this.obj["intangible_cultural_heritage_projects_id"],
					user_id
				};
              ...
              var praise_len = _this.praiseLen;
							this.$post('~/api/intangible_cultural_heritage_projects/set?intangible_cultural_heritage_projects_id=' + _this.obj["intangible_cultural_heritage_projects_id"], {
								praise_len
							}, (res) => {
								if(res.result){
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var intangible_cultural_heritage_projects_id = this.obj.intangible_cultural_heritage_projects_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "intangible_cultural_heritage_projects",
          source_field: "intangible_cultural_heritage_projects_id",
          source_id: intangible_cultural_heritage_projects_id
        }, (res) => {
		...
		watch:{
			obj:{
				...
				handler(){
					...
					this.loadInheritorNames()
									}
			}
		},
		computed: {
			inheritorDisplay() {
				if (this.inheritorNames && this.inheritorNames.length) {
					return this.inheritorNames.join("、");
				}
				return this.obj.personnel_name || "";
			}
		},
	};
</script>
