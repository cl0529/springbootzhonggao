      ...
      <el-row class="rows row2">
         ...
         <el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
                ...
                <el-button @click="reset()" style="margin-right: 74px;" class="search_btn_reset">重置</el-button>
                                                      <el-button v-if="$check_action('/inheriting_users/table','add') || $check_action('/inheriting_users/view','add')" @click="$router.push('./view?')" class="add">添加</el-button>
            <el-button v-if="$check_action('/inheriting_users/table','del') || $check_action('/inheriting_users/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
                        </el-col>
	    ...
	    <el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
                  ...
                  <el-table-column prop="username" label="用户名" width="120">
                        ...
                        <el-table-column fixed prop="nickname" label="昵称">
                                              ...
                                              <el-table-column prop="home_address" @sort-change="$sortChange" label="家庭住址"                                v-if="$check_field('get','home_address')" min-width="200">
                                </el-table-column>
                                              <el-table-column prop="id_picture" @sort-change="$sortChange" label="证件图片"                                v-if="$check_field('get','id_picture')" min-width="200">
                                      <template slot-scope="scope">
                      <el-image style="width: 100px; height: 100px" :src="$fullUrl(scope.row['id_picture'])"
                                :preview-src-list="[$fullUrl(scope.row['id_picture'])]">
      ...
      <el-table-column sortable prop="update_time" label="更新时间" min-width="200">
        <template slot-scope="scope">
          {{ $toTime(scope.row["update_time"],"yyyy-MM-dd hh:mm:ss") }}
                ...
                <el-table-column fixed="right" label="操作" min-width="120" v-if="$check_action('/inheriting_users/table','set') || $check_action('/inheriting_users/view','set') || $check_action('/inheriting_users/view','get')
					 ">
        

      <template slot-scope="scope">
        <router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
                     v-if="$check_action('/inheriting_users/table','set') || $check_action('/inheriting_users/view','set') || $check_action('/inheriting_users/view','get')"
                                 :to="'./view?user_id=' + scope.row['user_id']"
                                 size="small">
          <span>详情</span>
        </router-link>
                                                  		  		  		  	<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="changeSigning(scope.row, scope.$index)" v-if="$check_option('/inheriting_users/table', 'examine')">
		  		<span>审核</span>
		  	</el-button>
		        </template>
    </el-table-column>
  ...
  export default {
    ...
    data() {
      return {
        ...
        showModal: false,
        // 获取数据地址
        url_get_list: "~/api/inheriting_users/get_list?like=0",
        url_del: "~/api/inheriting_users/del?",

        // 字段ID
        field: "inheriting_users_id",
		dialogVisible: false,
		verifyIdx: 0,
		verifyItem: {},
		rules: {
        ...
        this.showModal = false;
			...
			assureVerify() {
				...
				examineForm.validate((valid) => {
					if (valid) {
						var url = "~/api/inheriting_users/set?inheriting_users_id=" + this.verifyItem['inheriting_users_id']
						_this.$post(url, {
