<template>
	<div class="diy_details page_cultural_and_creative_mall" id="cultural_and_creative_mall_details">
		...
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_cultural_and_creative_mall'>
							<div_cultural_and_creative_mall :obj="obj"></div_cultural_and_creative_mall>
...
<script>
	import div_cultural_and_creative_mall from "@/components/diy/div_cultural_and_creative_mall.vue";
	...
	export default {
		...
		components:{
			...
			bar_title,
			form_editor,
			div_cultural_and_creative_mall
		},
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "cultural_and_creative_mall_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/cultural_and_creative_mall/set?cultural_and_creative_mall_id=' + obj["cultural_and_creative_mall_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "cultural_and_creative_mall",
							source_field: "cultural_and_creative_mall_id",
							source_id: this.obj.cultural_and_creative_mall_id,
			...
			async get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.cultural_and_creative_mall_id;
							}
			},
						/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "cultural_and_creative_mall",
					source_field: "cultural_and_creative_mall_id",
					source_id: obj["cultural_and_creative_mall_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/cultural_and_creative_mall/set?cultural_and_creative_mall_id=' + _this.obj["cultural_and_creative_mall_id"], {
								praise_len
							}, (res) => {
								if(res.result){
