<template>
	<div class="diy_details page_inheritance_personnel" id="inheritance_personnel_details">
		...
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_inheritance_personnel'>
							<div_inheritance_personnel :obj="obj"></div_inheritance_personnel>
...
<script>
	import div_inheritance_personnel from "@/components/diy/div_inheritance_personnel.vue";
	...
	export default {
		...
		data(){
			return{
				...
				form_comment: {
					...
					source_field: "inheritance_personnel_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/inheritance_personnel/set?inheritance_personnel_id=' + obj["inheritance_personnel_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "inheritance_personnel",
							source_field: "inheritance_personnel_id",
							source_id: this.obj.inheritance_personnel_id,
			...
			async get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.inheritance_personnel_id;
														}
			},
													/**
			 * 初始化收藏状态
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "inheritance_personnel",
					source_field: "inheritance_personnel_id",
					source_id: obj["inheritance_personnel_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/inheritance_personnel/set?inheritance_personnel_id=' + _this.obj["inheritance_personnel_id"], {
								praise_len
							}, (res) => {
								if(res.result){
