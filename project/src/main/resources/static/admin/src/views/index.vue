...
<!--					</el-col>-->
				...
				<projectStatsPanel v-if="canViewProjectStats" />
...
<script>
	...
	import projectStatsPanel from "@/components/charts/project_stats_panel.vue";
	export default {
		mixins: [mixin],
		name: "Home",
		components: {
		...
		methods: {
			...
			async get_order_sum_price_sub(list, date, index, field) {
				...
				if (this.user.user_group != "管理员") {
					// Inheritors only analyze orders from goods they own.
					$where = '&sqlwhere=' + encodeURIComponent(`goods_id IN (SELECT goods_id FROM goods WHERE user_id = ${this.user.user_id})`)
				}
		...
		computed:{
			canViewProjectStats() {
				return this.user.user_group === "管理员" || this.user.user_group === "传承用户";
			},
			recognitionHeight(){
				return "830px"
			},
